...
class UserContext extends Component {
  ...
  sendCredentials = credentials => axios.post('/auth/login', credentials).then(response => response.data)
  ...
  render() {
    ...
    return <UserData.Provider value={props}>{this.props.children}</UserData.Provider>
  }
}
