...
class AssignStats extends Component {
  constructor(props) {
    super(props)
  ...
  assignStat = (num, statInd) => {
    ...
    if (
      statName === 'str' &&
      (['Half-Orc', 'Human', 'Dragpnborn'].includes(this.props.race) || ['Mountain Dwarf'].includes(this.props.subRace))
    ) {
      ...
      statName === 'dex' &&
      (['Elf', 'Halfling', 'Human'].includes(this.props.race) || ['Forest Gnome'].includes(this.props.subRace))
    ) {
      ...
      statName === 'con' &&
      (['Dwarf', 'Human', 'Half-Orc'].includes(this.props.race) || ['Stout', 'Rock Gnome'].includes(this.props.subRace))
    ) {
      ...
      statName === 'int' &&
      (['Tiefling', 'Human', 'Gnome'].includes(this.props.race) || ['High Elf'].includes(this.props.subRace))
    ) {
      ...
      statName === 'cha' &&
      (['Dragonborn', 'Human', 'Tiefling', 'Half-Elf'].includes(this.props.race) ||
  ...
  render() {
    return (
      <div style={{ height: '100%' }}>
        <div>
          Str: {this.props.str} Dex: {this.props.dex} Con: {this.props.con} Int: {this.props.int} Wis: {this.props.wis}{' '}
          Cha: {this.props.cha}
        </div>
        <AssignDisplay
...
export default withCharacter(AssignStats)
